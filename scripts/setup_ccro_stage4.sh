...
"${CONDA_BIN}" run -n "${CONDA_ENV}" python -m pip install \
  -r "${ROOT_DIR}/requirements-stage4.txt"
...
"${CONDA_BIN}" run -n "${CONDA_ENV}" python -m pytest \
  -p no:cacheprovider "${ROOT_DIR}/tests/planning" -q

echo "CCRO-NUBS stage-four environment is ready."
echo "Run: ${CONDA_BIN} run -n ${CONDA_ENV} python -m experiments.exp_ccro_stage4"
