...
"${CONDA_BIN}" run -n "${CONDA_ENV}" python -m pip install \
  -r "${ROOT_DIR}/requirements-stage1.txt"
...
"${CONDA_BIN}" run -n "${CONDA_ENV}" python -m pytest \
  -p no:cacheprovider "${ROOT_DIR}/tests/planning/test_nubs_stage1.py" -q

echo "CCRO-NUBS stage-one environment is ready."
echo "Run: ${CONDA_BIN} run -n ${CONDA_ENV} python -m experiments.exp_ccro_stage1"
