...
"${CONDA_BIN}" run -n "${CONDA_ENV}" python -m pip install \
  -r "${ROOT_DIR}/requirements-stage3.txt"
...
"${CONDA_BIN}" run -n "${CONDA_ENV}" python -m pytest \
  ...
  "${ROOT_DIR}/tests/planning/test_ccro_stage2.py" \
  "${ROOT_DIR}/tests/planning/test_ccro_stage3.py" -q

echo "CCRO-NUBS stage-three environment is ready."
echo "Run: ${CONDA_BIN} run -n ${CONDA_ENV} python -m experiments.exp_ccro_stage3"
