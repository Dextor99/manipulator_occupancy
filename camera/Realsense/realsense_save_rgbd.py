...
def mouse_callback(event, x, y, flags, param):
    global saved_count, save_path
    if event == cv2.EVENT_LBUTTONDOWN:
        color_image, depth_image = param
        depth_image_color = depth2RGB(depth_image)

        # 保存图片
        cv2.imwrite(os.path.join((save_path), "{:04d}_label7_RGB.png".format(saved_count)),
                    color_image)  # 保存RGB为png文件
        imsave(os.path.join((save_path), "{:04d}_label7_perfect_depth.tiff".format(saved_count)),
               depth_image)  # 保存深度图为tiff文件
        saved_count += 1
        cv2.imshow("save", np.hstack((color_image, depth_image_color)))
        print(f"Saved image {saved_count-1}")
...
def run():
    ...
    # 按照日期创建文件夹
    save_path = os.path.join(os.getcwd(), "test", time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime()))
    ...
    try:
        while True:
            ...
            # 显示图像
            combined = np.hstack((color_image, depth_image_color))
            cv2.imshow("live", combined)

            # 更新鼠标回调函数的参数
            cv2.setMouseCallback("live", mouse_callback, (color_image.copy(), depth_image.copy()))
