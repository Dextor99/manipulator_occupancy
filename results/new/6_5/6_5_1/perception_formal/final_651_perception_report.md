...
|---|---|---|---|
| S1 | 肘部/上臂附近静态障碍 | r04-r06 | r01-r03 实际更接近末端，不纳入区域识别统计 |
| S2 | 前臂附近静态障碍 | r04-r06 | r01-r03 实际更接近末端，不纳入区域识别统计 |
...
| scene | description | adopted trials | note |
|---|---|---|---|
| D1 | 横向经过肘部/上臂区域 | r01-r05 | 全部可用 |
| D2 | 斜向接近腕部/末端区域后离开 | r02-r06 | 原 r01 已删除，当前目录仅保留有效 D2 试次 |

区域不匹配试次已从正式 `trials/` 目录删除或仅在报告中标注，不进入自动汇总统计。
...
这些试次不影响主结论，因为补采后的 S1 r04-r06、S2 r04-r06 以及当前保留的 D2 r02-r06 已经覆盖目标区域。原 D2 r01 因区域不匹配已删除，避免后续误用。
