"""Fixed-time NUBS optimization with static full-body mesh risk."""
...
class StaticRiskNUBSOptimizer(FixedTimeNUBSOptimizer):
    """Optimize NUBS interpolation configurations against a static point cloud.

    The fixed-time trajectory mapping is linear in ``P_inner``.  Its sampled
    position sensitivity is therefore precomputed once and used to propagate
    finite-difference configuration risk gradients to waypoint variables.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.sensitivity_epsilon <= 0.0:
    ...
    def _build_sample_sensitivity(self, reference: np.ndarray) -> np.ndarray:
        variable_count = reference.size
        sensitivity = np.zeros(
            (len(self.risk_sample_times), DIMENSION, variable_count), dtype=np.float64
        )
    ...
    def cost_only(self, flat_points: np.ndarray) -> float:
        ...
        return float(self.lambda_smooth * energy + self.lambda_risk * risk.cost + penalty)
    ...
    def objective(self, flat_points: np.ndarray) -> tuple[float, np.ndarray]:
        ...
        if risk.gradient_q is None:
            raise RuntimeError("risk evaluator did not return configuration gradients")
        risk_gradient_flat = np.einsum(
        ...
        gradient = (
            ...
            + self.lambda_risk * risk_gradient_flat
            + penalty_gradient.ravel()
        )
        if not np.isfinite(cost) or not np.all(np.isfinite(gradient)):
            raise FloatingPointError("static-risk objective produced NaN or Inf")
    ...
    def check_gradient(
        ...
        numeric = np.zeros(points.size, dtype=np.float64)
        ...
        cosine = 1.0 if norm_product < 1.0e-16 else float(
            np.dot(analytic, numeric) / norm_product
        )
        return {
            "cost": float(cost),
    ...
    def optimize(self, p_inner_initial: np.ndarray | None = None) -> StaticRiskOptimizationResult:
        ...
        return StaticRiskOptimizationResult(
